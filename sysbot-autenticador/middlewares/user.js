...
User.prototype.ValidateFromWebhookToBot = function (req, res, next) {
    ...
    var _result = linq.from(_User).where("$._id == '" + req.body.meta.iduser + "' && $.idsubnetwork == '" + req.body.meta.idsubnetwork + "'").select().toArray();
    if (_result.length == 0) {
...
User.prototype.ValidateFromBotToWebhook = function (req, res, next) {
    ...
    var _result = linq.from(_User).where("$.iduser == '" + req.body.meta.iduser + "' && $.idsubnetwork == '" + req.body.meta.idsubnetwork + "'").select().toArray();
